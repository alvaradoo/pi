...
void run_spmd_pi(int n, double* error, double* timer, int NUM_THREADS) {
    double h, x, pi = 0., sum[NUM_THREADS]; /* sum is an array to track the local work of each thread */
    ...
    #pragma omp parallel 
    {
        ...
        for (i = id, sum[id] = 0.0; i < n; i += numthrds) {   /* Cyclic distribution for loop iterations */
            x = h * (i + .5);
            sum[id] += 4.*h/(1. + x*x);
        }
    }
